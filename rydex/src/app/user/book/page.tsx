...
function page() {
  ...
  const [pickUpSuggestions, setPickUpSuggetions] = useState<Place[]>([]);
  const [dropSuggestions, setDropSuggetions] = useState<Place[]>([]);
  ...
  const suggestion = (prop: Place) =>
    [prop.name, prop.city, prop.state, prop.country].filter(Boolean).join(",");
    ...
    <div className="min-h-screen bg-zinc-100 flex items-center justify-center px-4 py-10">
      ...
      >
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            {[0, 1, 2, 3].map((dot, idx) => (
              <motion.div
                key={idx}
                animate={{
                  width: idx < progress ? 20 : 8,
                  background: idx < progress ? "#09090b" : "#d4d4d8",
                }}
        ...
        <div className="bg-white rounded-3xl border border-zinc-200 shadow-[0_8px_40px_rgba(0,0,0,0.08)] overflow-visible">
          <div className="w-[90%] m-auto h-1 bg-zinc-900" />
          ...
          <div className="p-6 space-y-7">
                ...
                {VEHICLES.map((v, idx) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className={`relative p-3.5 rounded-2xl border flex items-center gap-3 text-left transition-all cursor-pointer duration-200 ${active ? "bg-zinc-900 border-zinc-900 shadow-lg" : "bg-zinc-50 border-zinc-200 hover:border-zinc-400"}`}
                    >
                      <div
                  ...
                  <div className="flex items-center gap-3 px-4 py-3.5 focus-within:bg-white rounded-t-2xl transition-colors">
                    ...
                    <input
                      onChange={(e) => {
                        ...
                        searchAddress(e.target.value, setPickUpSuggetions);
                      }}
                      value={pickUp}
                ...
                <div className="relative z-10">
                  <div className="flex items-center gap-3 px-4 py-3.5 focus-within:bg-white rounded-t-2xl transition-colors">
                    ...
                    <input
                      ...
                      value={drop}
                      placeholder={
                        pickUpCountry ? "Drop Location" : (
                          "Select Pick Up Loaction First"
                        )
                      }
                      className="flex-1 bg-transparent text-sm font-semibold text-zinc-900 placeholder:text-zinc-400 outline-none"
                    />
            ...
            >
              <motion.button
                ...
                className="w-full cursor-pointer h-14 rounded-2xl bg-zinc-900 hover:bg-black disabled:opacity-35 text-white font-black text-sm tracking-wide flex items-center justify-center gap-2.5 transition-colors shadow-lg disabled:shadow-none "
              >
                <span>Continue</span>
              </motion.button>
            </motion.div>
