    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between px-1 mb-1">
        <p className="text-xs font-semibold uppercase tracking-widest text-gray-400">
          {type === "partner" ?
            "Partners Reviews Queue"
          : type === "kyc" ?
            "Pending Video KYC Queue"
          : "Vehicle reviews Queue"}
        </p>

        <p className="text-xs text-gray-500">{data.length} Items</p>
      </div>
      ...
      {data.map((item: any, index: number) => {
        ...
        return (
          <motion.div
            ...
            className="bg-white border border-gray-400 rounded-2xl px-5 py-5 flex items-center justify-between gap-4 shadow-sm transition-shadow"
          >
